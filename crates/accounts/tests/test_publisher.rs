...
async fn test_publisher_publish_entry() -> Result<()> {
    ...
    let entry = Entry {  // Create an empty entry with the same pair
    ...
    let publisher_account = create_and_deploy_publisher_account(
        &mut client, 
        pair_word, 
        entry_as_word
    ).await?;
    ...
    let stored_entry = storage.get_map_item(1, pair_word).context("Failed to get entry map")?;
